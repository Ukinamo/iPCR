...
class SubmissionReviewController extends Controller
{
    public function show(Request $request, IpcrSubmission $submission): Response
    {
        ...
        return Inertia::render('Admin/SubmissionReview', [
    ...
    public function update(Request $request, IpcrSubmission $submission): RedirectResponse
    {
        ...
        $base = $request->validate([
            'action' => ['required', 'in:approve,return,reject,save'],
        ...
        if ($data['action'] === 'return') {
            ...
            if (strlen($feedback) < 20) {
                return back()->withErrors([
                    'supervisor_feedback' => 'Rejecting for revision requires clear guidance: please write at least 20 characters for the employee.',
                ]);
            }
        ...
        DB::transaction(function () use ($request, $admin, $submission, $data, $existingById, $template) {
            $incomingIds = [];
            $synced = collect();
            ...
            foreach ($synced as $item) {
                    ...
                    foreach (['rating_quality', 'rating_efficiency', 'rating_timeliness'] as $field) {
                        if (! isset($row[$field]) || ! is_numeric($row[$field])) {
                            ...
                            foreach ($quarters as $quarter) {
                                $pairs[] = [
                                    IpcrFormRatingCalculator::nullableWholeNumber($row["rating_q{$quarter}_target"] ?? null),
                                    IpcrFormRatingCalculator::nullableWholeNumber($row["rating_q{$quarter}_actual"] ?? null),
                                ];
                            }
                            $auto = IpcrFormRatingCalculator::totalsFromQuarterPairs($pairs);
                ...
                IpcrFormRatingCalculator::applyRowRatings(
                    $commitment,
                    $row,
                    $data['action'] !== 'approve',
                    \App\Support\IpcrIncludedQuarters::existingOrDefault($submission->included_quarters),
                );
                $commitment->refresh();
